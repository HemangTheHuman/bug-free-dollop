...
parse_runs.py
=============
One-time script to parse all tfevents files from the runs/d* directories
and produce a static runs_data.json in the project root.
...
Output:
    runs_data.json  (root of project)
"""
...
OUTPUT_FILE  = PROJECT_ROOT / "runs_data.json"

# Only process OCD runs (d* prefix)
RUN_PREFIX = "d"
...
def normalize_tag(tag: str) -> str:
    """Lower-case, replace spaces/slashes with underscores for consistency."""
...
def main() -> None:
    ...
    print(f"Found {len(run_dirs)} OCD run(s) to parse:\n")
    for d in run_dirs:
        print(f"  • {d.name}")
    print()
    ...
    for run_name, data in output["runs"].items():
        ...
        print(f"  {run_name}: {tags if tags else '(no scalar tags found)'}")
